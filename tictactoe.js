...
// reduce
// if human/computer gets a win vertically(ex
// xxx
// 000
// take an array that represents a row
// do a reduce on that array ex if you get three x's or three o's
// it should call three functions
// a function that calls for three in a row
	// given an array use reduce and take each array
// a function that checks for three in a column
// a function that checks for three in a diagnonal 
	// check for tie game
	// calling a sum function is any internal arrays represent three spots in a row
	

	)
var reduce = function(collection, acc, initial){
	each(collection, function(val){
		if(initial){
			initial = acc(initial, val);
		} else {
			initial = val;
		}
	});
